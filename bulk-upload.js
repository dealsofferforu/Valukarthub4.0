...
function bulkImportProducts() {
  ...
  importable.forEach(row => {
    if (row.categoryNeedsCreate && !categorySlugs.has(row.category)) {
      categories.push({ id: bulkStore.uid('cat'), name: row.categoryName, slug: row.category });
      categorySlugs.add(row.category);
    }
  });

  let products = bulkStore.getProducts();
  let added = 0;
  let updated = 0;
  const now = new Date().toISOString();

  importable.forEach(row => {
    const existing = row.action === 'update' ? bulkFindExistingProduct(products, row.title, row.category, row.amazonUrl) : null;
    const product = {
      id: existing?.id || bulkStore.uid('product'),
      title: row.title,
      category: row.category,
      secondaryCategories: existing?.secondaryCategories || [],
      price: row.price,
      mrp: row.mrp || existing?.mrp || 0,
      rating: row.rating || existing?.rating || 4.5,
      badge: row.badge || existing?.badge || 'discount',
      badgeText: row.badgeText || existing?.badgeText || bulkDefaultBadgeText('discount', row.price, row.mrp),
      art: row.art || existing?.art || '🛍️',
      image: row.image || existing?.image || '',
      amazonUrl: row.amazonUrl,
      views: row.views || existing?.views || 0,
      popular: row.views || existing?.popular || 0,
  ...
  try {
    bulkStore.saveCategories(categories);
    bulkStore.saveProducts(products);
  } catch (error) {
    console.error(error);
    showToast('Browser storage is full. Import fewer rows or connect a database.');
    return;
  }
