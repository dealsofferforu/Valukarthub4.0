...
function discountPercent(product) {
  const price = Number(product.price || 0), mrp = Number(product.mrp || 0);
...
function badgeClass(type) { return ['discount', 'flash', 'limited'].includes(type) ? 'badge-accent' : 'badge-green'; }
function renderProductMedia(product, detail = false) {
  if (product.image) return `<img class="${detail ? 'detail-product-image' : 'product-image'}" src="${escapeHtml(product.image)}" alt="${escapeHtml(product.title)}" loading="lazy" />`;
...
function renderCategories() {
  ...
  strip.innerHTML = `<button class="category-chip active" data-category="all">All Deals</button>${categories.map(category => `<button class="category-chip" data-category="${escapeHtml(category.slug)}">${escapeHtml(category.name)}</button>`).join('')}`;
}
function renderDeals() {
  ...
  grid.innerHTML = products.map(product => {
    ...
    return `<article class="deal-card" data-title="${escapeHtml(product.title)}" data-category="${escapeHtml(allCategories.join(' '))}" data-price="${Number(product.price || 0)}" data-discount="${discount}" data-popular="${Number(product.popular || product.views || 0)}">
      <div class="card-media"><div class="product-art">${renderProductMedia(product)}</div><div class="card-badges"><span class="badge ${badgeClass(product.badge)}">${escapeHtml(badgeText)}</span></div><button class="favorite" aria-label="Save deal">♡</button></div>
      <div class="card-body"><span class="card-category">${escapeHtml(categoryName(product.category))}</span><div class="card-title">${escapeHtml(product.title)}</div><div class="rating"><span class="stars">★★★★★</span><strong>${Number(product.rating || 4.5).toFixed(1)}</strong><span>ValueKart score</span></div><div class="price-row"><span class="price">${formatMoney(product.price)}</span>${product.mrp ? `<span class="mrp">${formatMoney(product.mrp)}</span>` : ''}${discount ? `<span class="discount">Save ${discount}%</span>` : ''}</div><div class="meta-row"><span>👁 ${views} views</span><span>Recently updated</span></div><a class="btn btn-accent" href="deal.html?product=${encodeURIComponent(product.id)}">Grab Deal</a></div>
    </article>`;
  }).join('');
}
function renderDetail() {
  ...
  const product = ValueKartStore.getProducts().find(item => item.id === productKey || item.legacyId === productKey);
  if (!product) return;
...
(async function initValueKart() {
  try {
    await ValueKartStore.loadPublic();
    renderCategories();
    renderDeals();
    renderDetail();
  } catch (error) {
    console.error(error);
    const grid = document.getElementById('dealGrid');
    if (grid) grid.innerHTML = '<div class="empty-state" style="display:block">Deals could not load. Please refresh the page.</div>';
    showToast('Could not load deals from database.');
  }
  ...
  document.getElementById('categoryStrip')?.addEventListener('click', event => {
    const chip = event.target.closest('.category-chip'); if (!chip) return;
    document.querySelectorAll('.category-chip').forEach(item => item.classList.remove('active'));
    chip.classList.add('active'); currentCategory = chip.dataset.category; refreshDeals();
  });
  sort?.addEventListener('change', () => {
    if (!grid) return;
    const cards = [...grid.querySelectorAll('.deal-card')], value = sort.value;
    cards.sort((a, b) => value === 'discount' ? +b.dataset.discount - +a.dataset.discount : value === 'low' ? +a.dataset.price - +b.dataset.price : +b.dataset.popular - +a.dataset.popular);
    cards.forEach(card => grid.appendChild(card)); refreshDeals();
  });
  ...
  document.querySelectorAll('[data-share]').forEach(btn => btn.addEventListener('click', () => {
    ...
    const routes = { whatsapp: `https://wa.me/?text=${title}%20${url}`, facebook: `https://www.facebook.com/sharer/sharer.php?u=${url}`, x: `https://twitter.com/intent/tweet?text=${title}&url=${url}` };
